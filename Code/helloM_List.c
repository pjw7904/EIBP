 ...
 //  printf("PayLoad --- %s",ctrlPayLoadA); 
	...
	while (1) {
		...
		if (failedLL_head != NULL) {
				...
				int mplrPayloadLen = 0;
				//NS buildPayloadRemoveAdvtsts  defined in endNW.c, declared in endNetworkUtilities.h 
				mplrPayloadLen = buildPayloadRemoveAdvts(mplrPayload, failedLL_head, 4); //NS - defined in endNW.c 
				if (mplrPayloadLen) {
					for (; loopCounter2 < interfaceListSize; loopCounter2++) {
						// MPLR TYPE 5.
						endNetworkSend(interfaceList[loopCounter2], mplrPayload, mplrPayloadLen);
		...
		if (time01-time00 > 0.25) { // Initially set to 2, 
			...
			if(ck==1){
				...
				printf("\nLabels in Quarantine table");
				neighbourFlag = 1;
				struct QuarantineTL *temp = Head_QTL;
				while(temp!=NULL){
					printf("\nTier_addr = %s \t port = %s \t Entry_At = %f \t Remove_At = %f",temp->tier,temp->port,temp->label_lost_time,temp->label_remove_time);
					temp = temp->next;
				}
			}
		...
		if (flagIP == 0) { //if message is recived from socket flagIP remains 0 and we enter the loop
			...
			if (ctrlIFName != NULL) {
				...
				if (isTierSet() == 0) {
					...
					if (checkDestStatus == false) {
						errorCount++;
						packetFwdStatus = sendQueryRequest(ipDestTemp,tierDest,nIP,bufferIP); //interAS
						//if(enableLogScreen)
...
//				 printf("\n");
	...
	//						 printf("      Number of Tier Address : %d, value of z : %d \n", tierAddrTotal, z);                          
							...
							if(neighbourFlag==1 && isNewPort==1){
        						...
        						printf("\n Sending NULL join request to %s",recvOnEtherPort);

            					ctrlLabelSend(MESSAGE_TYPE_JOIN,recvOnEtherPort, labelAssignmentPayLoad);
...
//								dataDecapsulationUDP(portNameTemp, decapsPacket, decapsPacketSize);
				...
				//exchange query messages when tier-3 node sends a request and tier-1, and when tier-1 node sends a response to tier-3 node
				if (checkMSGType == 3 && strcmp(recvOnEtherPort, "eth0") != 0) {
					uint8_t action = ethhead[15];
					if (action == MESSAGE_TYPE_QUERY_REQUEST){
					if(myTierValue==1){
						printf("\nNeed to respond to the query\n");
						...
						char *tempTier = getBrAddress(tierAddr);

						if (tempTier==NULL){
							tempTier = updateEndTierAddr(tierAddr);
						}
					...
					else if (action == MESSAGE_TYPE_QUERY_RESPONSE){
						if(myTierValue==3){
							int index = 16;
							uint8_t tierLen = ethhead[index];
							uint8_t tierAddr[tierLen+1];
					...
					else if (action == MESSAGE_TYPE_INTERAS_IP){
				...
				if (checkMSGType == 5 && strcmp(recvOnEtherPort, "eth0") != 0) {
					...
					if(action==MESSAGE_TYPE_ENDNW_INTER_IP){
						//printf("Received interAS ip address from %s with %d entries ",recvOnEtherPort,totalEntries);
						if(myTierValue==1){
							...
							while(totalEntries > 0) {
								...
								if (find_entry_LL(&ipAddr, tierAddr, 1) == 0) {
							...
							//printf("\nReceived interAS ip address = %s and its  and need to send it my parent %s via %s",inet_ntoa(ipAddr),parentTierAdd,fwdInterface);
					...
					if (strcmp(recvOnEtherPort,interAS_portName)==0){
						printf("Action was sent by a different AS, need to add the ip address into a interAS ip table");
						interAS_flag = 1;
					}
					...
					while (totalEntries > 0) {
						...
						if (action == MESSAGE_TYPE_ENDNW_ADD) {
							...
							if (find_entry_LL(&ipAddr, tierAddr, interAS_flag) == 0) {
								// insert info about index from which the packet has been received from.
					...
					else if(hasDeletions && action == MESSAGE_TYPE_ENDNW_INTER_IP){
						uint8_t *mplrPayload = allocate_ustrmem(IP_MAXPACKET);
						int mplrPayloadLen = 0;
						int z = 0;
						for (z = 14; z < index; z++, mplrPayloadLen++) {
				...
				if (checkMSGType == MESSAGE_TYPE_LABELS_AVAILABLE) {
					...
					for(;i<numberLabels;i++){
                      ...
                      //  printf("%s",label);
						...
						int check = insertTierAddr(label,recvOnEtherPort);	
                        if (check == 1)  {                    	
                            add_LL(label);
							print_entries_LL();
						}
		...
		while (argc > 0) { //the loop will continue until the number of arguments is greater than 1. 
			...
			if(myTierValue == 1)
			{
				...
				if (strcmp(opt, "-L") == 0) {
					...
					do {
						...
						char recv[5];
						// pass it to tier address list
						insertTierAddr(next, recv); //each node can have many tier label since it may derive from more than one parent node. All the values are stored in a list.
			...
			if (argc > 0) {
				...
				if (strcmp(opt, "-AS") == 0) {
					...
					argv++;
					char *port = argv[0];
					//interAS_portName = malloc(1 + strlen(port)); //allocate memory for interAS_portName
...
void checkEntriesToAdvertise() {
	...
	//interAS546
	if(interAS_flag==1){
		printf("Need to send the ip address to Tier-1 node");
...
void checkQuarantine(){
	struct QuarantineTL *temp = Head_QTL;
	struct QuarantineTL *prev = NULL;
	
	while(temp!=NULL){
		gettimeofday(&cur_time , NULL);
		double time = ((double)cur_time.tv_sec*1000000 + (double)cur_time.tv_usec)/1000000;

		if(time > temp->label_remove_time){
			removeLL(temp->tier);
			printf("\nRemoving %s after label_remove_time",temp->tier);
			
			if(temp==Head_QTL){
				Head_QTL = Head_QTL->next;
				free(temp);
				temp = Head_QTL;
				continue;
			}
                 ...
                 //   printf("\n checkMSGType=%d \n",checkMSGType);
                    ...
                    if (checkMSGType == MESSAGE_TYPE_LABELS_AVAILABLE) {
                        ...
                        for(;i<numberLabels;i++){
                            ...
                            // pass it to tier address list
                            insertTierAddr(label,recvOnEtherPort);
...
void add_LL(char label[10]){
	...
	if (endNode == 0) { //endNode is updated to 0, if we are at edge node, ie if edgeNode is 0
		//struct addr_tuple * myAddr1= (struct addr_tuple *) malloc(sizeof(struct addr_tuple)*(endNWCount * (myTotalTierAddress))); 
		//fp = fopen("logs.txt","a");
		//fprintf(fp,"T0->%d ->%d\n", endNWCount, myTotalTierAddress);
		
		//fprintf(fp,"cid = %d, IP = %s", cid, IP[0]);
		int index1, index2;
		int numTemp = 0;
		
		//fprintf(fp,"T->%s cidr %d\n", IP[0], cid[0]);
		//fclose(fp);
